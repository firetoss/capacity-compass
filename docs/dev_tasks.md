...
## M0 脚手架与规范
1. 目录与依赖
   - 创建 `src/sagescale/` 包、`tests/` 目录；添加 `__init__.py`。
...
## M1 配置加载与注册表
1. config_loader
   - 文件：`src/sagescale/config_loader.py`
...
2. models_registry
   - 文件：`src/sagescale/models_registry.py`
...
3. hardware_registry
   - 文件：`src/sagescale/hardware_registry.py`
...
4. scenarios_registry
   - 文件：`src/sagescale/scenarios_registry.py`
...
## M2 流水线实现（阶段 1–5）
1. normalizer（阶段 1）
   - 文件：`src/sagescale/pipeline/normalizer.py`
...
2. requirements（阶段 2）
   - 文件：`src/sagescale/pipeline/requirements.py`
...
3. hardware_filter（阶段 3）
   - 文件：`src/sagescale/pipeline/hardware_filter.py`
   - 输入：候选 GPU 列表 + 精度 + `weights_mem_bytes`。
   - 输出：可参与评估的 GPU 列表。
   - 验收：过滤 vendor/precision 权限；能装权重。

4. card_sizer（阶段 4）
   - 文件：`src/sagescale/pipeline/card_sizer.py`
   - 输入：`Requirements` + GPU。
   - 输出：每 GPU 的 `cards_mem/cards_compute/cards_needed/headroom`。
   - 验收：算力缺失时按显存给卡数并记录标记。

5. recommender（阶段 5）
   - 文件：`src/sagescale/pipeline/recommender.py`
...
1. evaluation_builder
   - 文件：`src/sagescale/pipeline/evaluation_builder.py`
...
## M4 LLM 生成（阶段 7，可选）
1. sales_summary 生成
   - 文件：`src/sagescale/pipeline/llm_summary.py`
