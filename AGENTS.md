...
6. **注释与文档**
   ...
   - 对复杂公式或经验系数，需在 docstring 中指明：
     - 对应设计文档的章节；
     - 或外部论文/文档的来源。
   - 对于核心计算/逻辑（normalizer、requirements、card_sizer、recommender、evaluation_builder 等），在关键代码块前给出简洁的行内注释，解释变量含义和推导思路，避免仅凭代码猜测。
   - 注释遵循“说明原因，不复述代码”原则，并保持与设计文档同步；如公式或行为发生变化，需同步更新注释和文档引用。
